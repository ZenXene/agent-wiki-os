...
impl RefinementEngine {
    pub async fn process(raw_data: &str, wiki_root: &Path, source_agent: &str, mode: ProcessMode, custom_output: Option<String>) -> anyhow::Result<String> {
        ...
        // Campaign 1: Context Protection (Prevent Context Explosion)
        let max_chars = 150_000; // Roughly 30-40k tokens depending on language
        let safe_data = if raw_data.len() > max_chars {
            println!("⚠️ WARNING: Input data exceeds safe context window ({} > {} chars). Truncating to prevent LLM crash.", raw_data.len(), max_chars);
            let mut truncated = String::from(&raw_data[..max_chars]);
            truncated.push_str("\n\n... [CONTENT TRUNCATED DUE TO SIZE LIMITS. PLEASE INGEST SMALLER DIRECTORIES OR SPECIFIC FILES] ...");
            truncated
        } else {
            raw_data.to_string()
        };
        ...
        let prompt = match mode {
            ProcessMode::WorkingMemory => {
                format!(
                    ...
                    Raw Chat History:\n
                    {}", source_agent, output_instruction, source_agent, safe_data
                )
            },
